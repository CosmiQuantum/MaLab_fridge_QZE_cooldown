...
device_name = 'squill'
substudy_txt_notes = ('script longitudinal_qze_008_find_higher_transitions.py')

study = 'higher_spec_transitions'
...
for QubitIndex in Qs_to_look_at:
    while j<100:
        sub_study = f'qubit_' + str(QubitIndex)
        ...
        ef_res_spec = ResonanceSpectroscopyEF(QubitIndex, tot_num_of_qubits, studyDocumentationFolder, j,
        ...
        saver_ef_res = Data_H5(optimizationFolder, ef_res_data, j, save_r)  # save
        saver_ef_res.save_to_h5('res_ef')
        del saver_ef_res
        del ef_res_data
        gc.collect()
        ef_res_data = create_data_dict(res_keys, save_r,
                                       list_of_all_qubits)  # initialize again to a blank for saftey

        del ef_res_spec

        experiment.readout_cfg['res_freq_ef'] = ef_res_freqs
        ...
        ef_q_spec = EFQubitSpectroscopy(QubitIndex, tot_num_of_qubits, studyDocumentationFolder, j,'None', save_figs, plot_fit=True, experiment=experiment,
                                       live_plot=live_plot, verbose=verbose, logger=rr_logger, unmasking_resgain=unmask)
