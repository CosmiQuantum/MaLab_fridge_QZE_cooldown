...
class LengthRabiExperimentQZE:
    def __init__(self, QubitIndex, number_of_qubits, outerFolder, round_num, signal, save_figs, experiment = None,
        ...
        if self.QZE:
            ...
            self.experiment.qubit_cfg['qubit_freq_ge_starked'] = self.experiment.qubit_cfg['qubit_freq_ge_starked'][self.QubitIndex]
    ...
    def run_QZE_one_starked_qfreq(self, wait_for_res_ring_up=False,
                                  exp=None,optimizationFolder=None,subStudyFolder=None, three_pulse_binary=False):
        qubit_length_ge_loop = np.linspace(self.config['start'], self.config['stop'],self.config['steps'])
        lengths = []
        I = []
        Q = []
        ...
        for row_idx, length in enumerate(qubit_length_ge_loop):
            updated_config = deepcopy(self.config)
            updated_config['qubit_length_ge'] = length #length of qubit pulse increases as you send more photons to drive rabi, i wonder if stark shift changes during this sweep?

            # if updated_config['qubit_pi_len']+0.1> length > updated_config['qubit_pi_len']: #to small to get a proper starked freq probably because qspec pulse will be tiny
            #     updated_config['qubit_freq_ge_starked'] = updated_config['qubit_freq_ge']
            # elif length>updated_config['qubit_pi_len']:
            #     res_freq_stark = copy.deepcopy(updated_config['res_freq_ge'])
            #     res_freq_stark.append(res_freq_stark[self.QubitIndex])
            #
            #     res_phase_stark = copy.deepcopy(self.experiment.readout_cfg['res_phase'])
            #     res_phase_stark.append(res_phase_stark[self.QubitIndex])
            #     res_stark_shift_2D = QZEStyleResStarkShift2D(self.QubitIndex, tot_num_of_qubits, optimizationFolder,
            #                                                  res_freq_stark,
            #                                                  res_phase_stark, self.save_figs, experiment=self.experiment,
            #                                                  zeno_stark_pulse_gain=updated_config['qubit_gain_ge'])
            #     stark_res_I, stark_res_Q, stark_res_qu_freq_sweep, starked_freq, sys_config_stark_res = res_stark_shift_2D.run(length)
            #     starked_freqs.append(starked_freq)
            #     starked_qspec_I.append(stark_res_I)
            #     starked_qspec_Q.append(stark_res_Q)
        ...
        #
        # if type(qubit_length_ge_loop) is list:
        #     qze_pulse_gains=np.array(qubit_length_ge_loop)
        #
        # plot = axes[0]
        # plot.set_box_aspect(1)
        # plt.colorbar(plot.pcolormesh(stark_res_qu_freq_sweep, qubit_length_plot, starked_qspec_I, cmap="viridis"), ax=plot, shrink=0.7)
        # plot.set_title("I [a.u.]")
        # plot.set_ylabel("qubit pulse length (And res pulse len w 0 gain) [a.u.]")
        # plot.set_xlabel("qubit pulse frequency [MHz]")
        #
        # plot = axes[1]
        # plot.set_box_aspect(1)
        # plt.colorbar(plot.pcolormesh(stark_res_qu_freq_sweep, qubit_length_plot, starked_qspec_Q, cmap='viridis'), ax=plot, shrink=0.7)
        ...
        # plot = axes[2]
        # plot.set_box_aspect(1)
        # plt.colorbar(plot.pcolormesh(stark_res_qu_freq_sweep, qubit_length_plot, np.sqrt(np.square(starked_qspec_I) + np.square(starked_qspec_Q)), cmap='viridis'), ax=plot,
        #              shrink=0.7)
        ...
        #####simple figure
        # plt.figure()
        # plt.plot(qubit_length_ge_loop,starked_freqs)
        # plt.xlabel('qubit_pulse_lengths')
    ...
    def run_rabi_chevron_after_pi_pulse(self, wait_for_res_ring_up=False,
                                  exp=None,optimizationFolder=None,subStudyFolder=None, three_pulse_binary=False):
        ...
        for row_idx, length in enumerate(qubit_length_ge_loop):
            updated_config = deepcopy(self.config)
            updated_config[
                'qubit_length_ge'] = length  # length of qubit pulse increases as you send more photons to drive rabi, i wonder if stark shift changes during this sweep?
            print('updated_config[qubit_gain_ge]: ', round(updated_config['qubit_gain_ge'], 4),
                  'updated_config[qubit_length_ge]: ', round(updated_config['qubit_length_ge'], 4),
                  ' updated_config[res_gain_qze]: ', [round(float(n), 4) for n in updated_config['res_gain_qze']])

            amp_rabi = RabiChevronAfterPiProgram(
                self.experiment.soccfg,
                reps=updated_config['reps'],
                final_delay=updated_config['relax_delay'],
                cfg=updated_config
...
class RabiChevronAfterPiProgram(AveragerProgramV2):
    def _initialize(self, cfg):
        ...
        self.add_pulse(ch=qubit_ch, name="qubit_pulse_pi_len",  #after pi pulse len, this is the normal non zeno freq drive
                       style="const",
                       length=cfg['qubit_pi_len'],  # total_drive_length,
                       freq=cfg['qubit_freq_ge'],  # [0] # only should be one value
                       phase=cfg['qubit_phase'],
                       gain=cfg['qubit_gain_ge'])  # ramp it up here

        if cfg['qubit_length_ge'] > cfg['qubit_pi_len']+0.01:
            self.add_pulse(ch=qubit_ch, name="offset_qubit_pulse",
                           style="const",
                           length=cfg['qubit_length_ge']-cfg['qubit_pi_len'],  # total_drive_length,
                           freq=cfg['qubit_freq_chevron_detuned_ge'],  # _starked
                           phase=cfg['qubit_phase'],
                           gain=cfg['qubit_gain_ge'])
    ...
    def _body(self, cfg):
        # drive the qubit on the qubit channel (list with length 6)
        if cfg['qubit_length_ge'] <= cfg['qubit_pi_len'] + 0.01:
            self.pulse(ch=cfg["qubit_ch"], name="qubit_pulse", t=0)  # just drive with normal freq
        else:
            self.pulse(ch=cfg["qubit_ch"], name="qubit_pulse_pi_len", t=0)  # regular w01 pi pulse first
            self.delay_auto(t=0, tag='waiting_pi')  # wait to finish
            self.pulse(ch=cfg["qubit_ch"], name="offset_qubit_pulse",
                       t=cfg['res_ring_up_time'])  # play offset freq qubit drive
        self.delay_auto(t=0, tag='waiting')  # auto wait for those pulses to be done
...
class QZE_constant_pulse_gnd_RabiProgram(AveragerProgramV2):
    ...
    def _body(self, cfg):
        ...
        if cfg['qubit_length_ge'] <= cfg['qubit_pi_len']+0.01:
            ...
            #self.delay_auto(t=cfg['res_ring_up_time'])  # wait for that one to stop, and then for ring down
        self.delay_auto(t=0, tag='waiting')  # auto wait for those pulses to be done
        self.delay(t=cfg['res_ring_up_time'])  # wait for ring down that will happen in qze, but of course the resonator doesnt actually have photons here
        #immediately after the qubit pulse ends, trigger the readout resonator pulse (9us long).
        self.pulse(ch=cfg['res_ch'], name="res_pulse")
    ...
    def _initialize(self, cfg):
        ...
        if cfg['qubit_length_ge'] > 0.11*3+0.01:
            self.add_pulse(ch=qubit_ch, name="starked_qubit_pulse",
                           ...
                           freq=cfg['qubit_freq_ge_starked'],  # [0] # only should be one value
                           phase=cfg['qubit_phase'],
                           gain=cfg['qubit_gain_ge'])  # ramp it up here
