...
for QubitIndex in Qs_to_look_at:
    gain_str = str(gain_updated[1]).replace('.', 'p')
    sub_study = f'gain' + gain_str
    data_set = f'qubit_' + str(
        QubitIndex) + 'round0'  # + '_' +datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ...
    gain_str = str(gain_updated[0]).replace('.', 'p')
    sub_study = f'gain' + gain_str
    data_set = f'qubit_' + str(
        QubitIndex) +'round0' # + '_' +datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ...
    for repeat_round in range(0,10000):
        for g in gain_updated:
            gain_str = str(g).replace('.', 'p')
            sub_study = f'gain' + gain_str
            data_set = f'qubit_' + str(
            ...
            exp.readout_cfg['res_gain_qze'] = g
            ...
            t1 = T1Measurement_with_Zeno_loop(QubitIndex, tot_num_of_qubits, studyDocumentationFolder, j, signal,
                                              ...
                                              qubit_to_increase_reps_for=qubit_to_increase_reps_for,
                                              multiply_qubit_reps_by=multiply_qubit_reps_by,
                                              verbose=verbose, logger=rr_logger, unmasking_resgain=unmask,zeno_pulse_gain=g
                                              )
            t1_est, t1_err, t1_I, t1_Q, t1_delay_times, q1_fit_exponential, sys_config_t1, \
                ss_Q_e_t1, ss_Q_g_t1, ss_I_e_t1, ss_I_g_t1, I_shots_t1, Q_shots_t1 = t1.run_single_gain(
        ...
        #         ss_I_e_t2e, ss_I_g_t2e, I_shots_t2e, Q_shots_t2e = t2e.run_single_gain(
