...
class AmplitudeRabiExperiment:
    def __init__(self, QubitIndex, number_of_qubits, outerFolder, round_num, signal, save_shots=False, save_figs = True, experiment = None,
                 ...
                 projective_readout_pulse_len_us=9,  time_between_projective_readout_pulses=None, expt_name = "power_rabi_ge",chevron=False,
    ...
    def run(self, thresholding=False, scaling=False):
        ...
        if self.QZE:
            ...
            if not self.chevron:
                ...
                amp_rabi = AmplitudeRabiChevronProgram(self.experiment.soccfg, reps=self.config['reps'],
                                                final_delay=self.config['relax_delay'], cfg=self.config)
        ...
        if scaling:
            ...
            if self.chevron:
                ...
                q1_fit_cosine, pi_amp = None, None
            else:
    ...
    def run_active_reset(self, scaling=True, control_test=False):
        if scaling:
            ...
            ss_config = {**q_config[self.Qubit], **ss_exp_cfg}
            print('performing single shot for g-e calibration')
            ssp_g = SingleShotProgram_g(self.experiment.soccfg, reps=1, final_delay=4000,
                                        cfg=ss_config)
            iq_list_g = ssp_g.acquire(self.experiment.soc, rounds=1, progress=True)
            ssp_e = SingleShotProgram_e(self.experiment.soccfg, reps=1, final_delay=4000,
                                        cfg=ss_config)
            iq_list_e = ssp_e.acquire(self.experiment.soc, rounds=1, progress=True)
    ...
    def plot_results_chevron(self, I, Q, lens, freqs,
        ...
        try:
            ...
            ax.set_xlabel("Qubit drive pulse gain", fontsize=20)
    ...
    def plot_results(self, I, Q, gains, config=None, fig_quality=100, scaling=False, Ie=None, Ig=None, Qe=None,
        ...
        try:
            if scaling:
                ...
                ax1.plot(gains, ydata, label="Gain (a.u.)", linewidth=2)
...
class RabiWithActiveResetControlTest(AveragerProgramV2):
    ...
    def _body(self, cfg):
        ...
        self.pulse(ch=cfg['res_ch'], name="res_pulse", t=0)  # play probe pulse
        self.trigger(ros=[cfg['ro_ch']], pins=[0], t=cfg['trig_time'])
class RabiWithActiveReset(AveragerProgramV2):
    def _initialize(self, cfg):
        ro_ch = cfg['ro_ch']
        res_ch = cfg['res_ch']
        qubit_ch = cfg['qubit_ch']
    ...
    def _body(self, cfg):
        ...
        # Convert blob-center I values into the same integrated units used by read_and_jump
        ...
        for i in range(n_resets):
            ...
            # Region 1: left of |g> center --> skip everything
            # if I < g_thresh: jump to done
            self.read_and_jump(
                ro_ch=cfg['ro_ch'],
                component='I',
                threshold=g_thresh,
                test="<",
                label=f"reset_done_completely_1{i}"
            )

            # If we are here, then I >= g_thresh.
            # Now check whether we are between g and e, or to the right of e.

            # Region 2: between g center and e center --> measure again, then redo logic
            # Region 3: right of e center --> pi, then measure again, then redo logic
            #
            # We detect Region 2 by testing I < e_thresh.
            self.read_and_jump(
                ro_ch=cfg['ro_ch'],
                component='I',
                threshold=e_thresh,
                test="<",
                label=f"middle_region_1{i}"
            )
            ...
            # redo read_and_jump from this new measurement:
            # if now < g center, we're done
            self.read_and_jump(
                ro_ch=cfg['ro_ch'],
                component='I',
                threshold=g_thresh,
            ...
            # if now between g and e, go to middle handling
            self.read_and_jump(
                ro_ch=cfg['ro_ch'],
                component='I',
                threshold=e_thresh,
                test="<",
                label=f"middle_region_retry_1{i}"
            )

            # still right of e after retry: do another pi+measure+check sequence
            self.pulse(ch=self.cfg["qubit_ch"], name="pi_pulse", t=0)
            self.delay_auto(t=5)
            ...
            # if still right of e after the second retry, give up and continue
            self.jump(label=f"reset_done_completely_1{i}")

            # -------- Between |g> center and |e> center --------
            self.label(f"middle_region_1{i}")
            # only measure again
            self.pulse(ch=cfg['res_ch'], name="res_pulse", t=0)
            self.trigger(ros=[cfg['ro_ch']], pins=[0], t=cfg['trig_time'])
            self.wait_auto(0.01, gens=True, ros=True)
            ...
            self.read_and_jump(
                ...
                threshold=e_thresh,
                test="<",
                label=f"additional_reset_1{i}"
            )

            # if now right of e after the extra measurement, apply pi once
            self.pulse(ch=self.cfg["qubit_ch"], name="pi_pulse", t=0)
            self.delay_auto(t=5)
            self.jump(label=f"reset_done_completely_1{i}")

            # -------- Came from right branch and landed in middle after retry --------
            self.label(f"middle_region_retry_1{i}")

            # additional reset block for "still in the middle"
            self.label(f"additional_reset_1{i}")

            # do one more reset action
            self.pulse(ch=self.cfg["qubit_ch"], name="pi_pulse", t=0)
            self.delay_auto(t=5)

            # measure again after additional reset
            self.pulse(ch=cfg['res_ch'], name="res_pulse", t=0)
            self.trigger(ros=[cfg['ro_ch']], pins=[0], t=cfg['trig_time'])
            self.wait_auto(0.01, gens=True, ros=True)
            self.resync()
            ...
            # final recheck after the additional reset
            self.read_and_jump(
                ro_ch=cfg['ro_ch'],
                component='I',
                threshold=g_thresh,
            ...
            # if still to the right after the additional reset, just move on
            self.label(f"reset_done_completely_1{i}")
        ...
        for i in range(n_resets):
            ...
            self.read_and_jump(
                ...
                threshold=g_thresh,
                test="<",
                label=f"reset_done_completely_{i}"
            )
            ...
            self.read_and_jump(
                ...
                threshold=e_thresh,
                test="<",
                label=f"middle_region_retry_{i}"
            )
            ...
            # if still right of e after the second retry, give up and continue
            self.jump(label=f"reset_done_completely_{i}")

            # -------- Between |g> center and |e> center --------
            self.label(f"middle_region_{i}")
            # only measure again
            self.pulse(ch=cfg['res_ch'], name="res_pulse", t=0)
            self.trigger(ros=[cfg['ro_ch']], pins=[0], t=cfg['trig_time'])
            self.wait_auto(0.01, gens=True, ros=True)
            ...
            # -------- Came from right branch and landed in middle after retry --------
            self.label(f"middle_region_retry_{i}")

            # additional reset block for "still in the middle"
            self.label(f"additional_reset_{i}")
            ...
            # if still to the right after the additional reset, just move on
            self.label(f"reset_done_completely_{i}")
    ...
    # def _body(self, cfg):
    #     #one reset before the pi pulse to bring to 0 for this experiment
    #     n_resets1 = 3
    #     for n in range(n_resets1):
    #         self.pulse(ch=cfg['res_ch'], name="res_pulse", t=0)
    #         self.trigger(ros=[cfg['ro_ch']], pins=[0], t=cfg['trig_time'])
    #         self.wait_auto(0.01, gens=True, ros=True)
    #         self.resync()
    ...
    #                            test="<", label=f'skip_reset_before_pi{n}')
    #         self.pulse(ch=self.cfg["qubit_ch"], name="pi_pulse", t=0)
    #         self.delay_auto(t=5)
    #         self.label(f'skip_reset_before_pi{n}')
    #
    #     #do the rabi pulse
    #     self.pulse(ch=self.cfg["qubit_ch"], name="qubit_pulse", t=0)
    #     self.delay_auto(t=0.0, tag='waiting')
...
class AmplitudeRabiChevronProgram(AveragerProgramV2):
    def _initialize(self, cfg):
        ro_ch = cfg['ro_ch']
        res_ch = cfg['res_ch']
        qubit_ch = cfg['qubit_ch']
