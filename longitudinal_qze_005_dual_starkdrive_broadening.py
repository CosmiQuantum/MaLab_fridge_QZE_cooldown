...
for QubitIndex in Qs_to_look_at:
    sub_study = '_' +datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ...
    experiment.qubit_cfg['qubit_freq_ge'] = float(3095.47)
    experiment.qubit_cfg['pi_amp'] =  experiment.qubit_cfg['pi_amp'][QubitIndex]
    bare_non_offset_res_freq =7287.59
    ...
    for repeat_round in range(0,1000):
        ...
        from section_004_qubit_spec_ge_dual_stark_drive import QubitSpectroscopyDualStark

        ############################################# Res drive ########################################################
        q_spec = QubitSpectroscopyDualStark(QubitIndex, tot_num_of_qubits, studyDocumentationFolder, j,
        ...
        #  gains_qspec_res_stark) = q_spec.run_res_stark(scaling=True,qze_pulse='const')
        #
        # qspec_data = create_data_dict(qspec_keys, save_r, list_of_all_qubits)
        #
        # qspec_data[QubitIndex]['Dates'][0] = (
