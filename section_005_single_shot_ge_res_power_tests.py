...
###################################################################################################################################
# Separate g and e per each experiment defined.
class SingleShotProgram_g_test(AveragerProgramV2):
    def _initialize(self, cfg):
        ro_chs = cfg['ro_ch']
        res_ch = cfg['res_ch']
        qubit_ch = cfg['qubit_ch']
        ...
        self.add_pulse(ch=res_ch, name="res_pulse_test", ro_ch=ro_chs,
                       style="const",
                       length=cfg["res_length"],
                       freq=cfg['res_freq_ge'],
                       phase=cfg['ro_phase'],
    ...
    def _body(self, cfg):
        self.delay_auto(0.01)
        self.pulse(ch=cfg['res_ch'], name="res_pulse_test", t=0)  # play probe pulse
...
class SingleShotProgram_e_test(AveragerProgramV2):
    def _initialize(self, cfg):
        ro_ch = cfg['ro_ch']
        res_ch = cfg['res_ch']
        qubit_ch = cfg['qubit_ch']
    ...
    def _body(self, cfg):
        self.pulse(ch=cfg['res_ch'], name="res_pulse_test", t=0)  # play test probe pulse
        self.delay_auto(cfg["relax_delay_test"])
...
class SingleShot:
    ...
    def hist_ssf(self, data=None, cfg=None, plot=True,  fig_quality = 100):
        ...
        if plot == True:
            ...
            axs[0].scatter(ig, qg, label=f'g: avgIg={np.mean(ig)}', color='b', marker='*', alpha=0.2)
            axs[0].scatter(ie, qe, label=f'e: avgIe={np.mean(ie)}', color='r', marker='*', alpha=0.2)
