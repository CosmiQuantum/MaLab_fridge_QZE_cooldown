...
#sys.path.append(os.path.abspath("/home/louduser/Qickcodes2/QICK_Qubit_LabSuite/src/"))
#sys.path.append(os.path.abspath("/home/louduser/Qickcodes2/QICK_Qubit_LabSuite/"))
#sys.path.append(os.path.abspath("/home/louduser/Qickcodes2/"))
sys.path.append(os.path.abspath("/home/louduser/.local/lib/python3.9/site-packages/QICK_Qubit_LabSuite/src/"))
...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------
run_num = 7
run_name = f'run7B/Silicon' # this is for temps analysis, for coherence analysis it's defined in its respective section
...
base_dir_run7B = r"C:/data/QICK_data/run7B/Silicon/Effective_Temp4/Q345" # up to study name                                      

# Arianna's local analysis                                                                                             
#base_dir_run8 = r"C:\Users\Arianna\Documents\Grad\Research\CosmicQ\QUIET\run8" # up to study name                      

# for Arianna's local analysis: (specify date folder) ## LOUD substudy                                                                  
target_dates_qtemps_RPM_run7B = ["2026-03-08_17-05-40"]

# To re-make and save RPM RR plots                                                                                     
outerFolder_qtemps_plots_RR_run7B = r"C:/home/louduser/Molenaar/APS_2026_Data"
...
# For RPM Analysis                                                                                                     
outerFolder_qtemps_plots_run7B = r"C:/home/louduser/Molenaar/APS_2026_Data/Rabi" # Inside each analysis function, a subfolder will be defined                                      
...
#-----------------------------------------------------------------------run 5(recent)------------------------------------------------------------
# Base path of where the data is stored up to the Study Name (round_robin_benchmark)
base_dir_run5 = "/data/QICK_data/run5/Silicon/Effective Temp/"

# for 24hr AB data
target_dates_qtemps_RPM_run5 = ["2026-01-15_22-35-59"]

# To re-make and save RPM RR plots <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
outerFolder_qtemps_plots_RR_run5 =  "/data/QICK_data/run7/6transmon/round_robin_benchmark/AB_paper_data/benchmark_analysis_plots/RPM_RR_plots"
#
# For RPM Analysis
outerFolder_qtemps_plots_run5 = "/data/QICK_data/run7/6transmon/round_robin_benchmark/AB_paper_data/benchmark_analysis_plots/RPM_analysis" # Inside each analysis function, a subfolder will be defined

# Substudy name on the file path, doesn't have to be exact, it will look for these key terms in the name
filter_keywords_run5 = ['Q345Temp']
...
if run_num == 7:
    Science_Qubits = [2, 4]
    base_dir = base_dir_run7B
    #filter_keywords = filter_keywords_run7B
    outerFolder_qtemps_plots_RR = outerFolder_qtemps_plots_RR_run7B
    outerFolder_qtemps_plots = outerFolder_qtemps_plots_run7B
    target_dates_qtemps_RPM = target_dates_qtemps_RPM_run7B
...
paths_SSFmethods_run5 = [
    "/data/QICK_data/run5/Silicon/Effective Temp/Q345Temp/2026-01-15_22-35-59"
]

path_saveplots_fits_run5 = "/data/QICK_data/run5/Silicon/Effective Temp/fits_plots" # where to save ssf plots to check gaussian fits
path_saveplots_ssf_qtemps_vsT_run5 = "/exp/cosmiq/data/home/cosmiq/Analysis_on1hw_temporary/acolonce/QTemperatures/Plots/Qtemps_SSFmethod/run5_analysis" # to save qubit temps vs time via ssf methods
...
#------------------------------------------------------------------------------ Assign func variables depending on run number ---------------------------------------
if run_num == 5: # already defined Science_Qubits above
