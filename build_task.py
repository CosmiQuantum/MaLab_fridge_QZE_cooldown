...
def add_single_qubit_experiment(expt_cfg, expt_name, QubitIndex):
    ...
    if "stop" in exp_cfg:
        ...
        start_cfg = exp_cfg["start"]
        stop_cfg = exp_cfg["stop"]
        start = (start_cfg if isinstance(start_cfg, (int, float))
                 else start_cfg[QubitIndex])
        stop = (stop_cfg if isinstance(stop_cfg, (int, float))
                else stop_cfg[QubitIndex])
        ...
        if isinstance(exp_cfg.get("steps"), (list, tuple, np.ndarray)):
            exp_cfg["steps"] = exp_cfg["steps"][QubitIndex]
    ...
    elif (expt_name == 'Ramsey_ge' or expt_name == 'Ramsey_ge_zeno'or expt_name == 'Ramsey_ge_zeno_correction' or expt_name == 'Ramsey_ge_correction'
          or expt_name == 'SpinEcho_ge_zeno' or expt_name == 'SpinEcho_ge' or expt_name == 'T1_ge' or expt_name == 'T1_fg'
