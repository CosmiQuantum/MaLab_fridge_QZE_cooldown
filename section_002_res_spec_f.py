...
class SingleToneSpectroscopyProgram(AveragerProgramV2):
    def _initialize(self, cfg):
        ...
        self.add_pulse(ch=qubit_ch, name="qubit_pulse_ge",
                       style="arb",
                       envelope="ramp",
        ...
        self.add_gauss(ch=qubit_ch, name="ramp_ef", sigma=cfg['sigma_ef'], length=cfg['sigma_ef'] * 4, even_length=False)
    ...
    def _body(self, cfg):
        self.pulse(ch=self.cfg["qubit_ch"], name="qubit_pulse_ge", t=0) # play pi pulse ge
...
class ResonanceSpectroscopyFH:
    ...
    def plot_results(self, fpts, fcenter, amps, reloaded_config=None, fig_quality=100):
        ...
        if self.experiment is not None:
            plt.suptitle(f"Resonator spectroscopy F {self.config['reps']}*{self.config['rounds']} avgs",
                         fontsize=24, y=0.95)
        else:
            plt.suptitle(f"Resonator spectroscopy F {reloaded_config['reps']}*{reloaded_config['rounds']} avgs",
                         fontsize=24, y=0.95)
