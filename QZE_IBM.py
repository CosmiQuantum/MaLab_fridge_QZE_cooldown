...
for QubitIndex in Qs_to_look_at:
    ...
    pulse_gains=np.linspace(0.005, 0.3, 10)
    ...
    for gain in pulse_gains:
        ...
        if run_flags["t1"]:
            try:
                t1 = T1Measurement_with_Zeno(QubitIndex, tot_num_of_qubits, outerFolder, j, signal, save_figs,
                                   experiment = experiment,
                                   live_plot = live_plot, fit_data = fit_data,
                                   increase_qubit_reps = increase_qubit_reps,
                                   qubit_to_increase_reps_for = qubit_to_increase_reps_for,
                                   multiply_qubit_reps_by = multiply_qubit_reps_by,
                                   verbose = verbose, logger = rr_logger, zeno_pulse_gain=gain)
