...
class SingleShotProgram_g_active_reset(AveragerProgramV2):
    ...
    def _active_reset_block(self, cfg):
        ...
        for i in range(n_resets):
            ...
            # NOTE: feedback reads ro_ch[0]; for multi-qubit use this qubit's own channel
            self.read_and_jump(ro_ch=cfg['ro_ch'][0],
                               component='I',
                               threshold=int(np.round(
                                   cfg["threshold"] * self.soccfg.us2cycles(cfg['res_length'], ro_ch=cfg['ro_ch'][0]))),
...
class SingleShot:
    ...
    def run(self, active_reset=False):
        ...
        if active_reset:
            ssp_e = SingleShotProgram_e_active_reset(self.experiment.soccfg, reps=1, final_delay=self.config['relax_delay'], cfg=self.config)
        else:
        ...
        fid, angle = self.plot_results(g_shots, e_shots, self.QubitIndex, active_reset=active_reset)
    ...
    def plot_results(self, iq_list_g, iq_list_e, QubitIndex,  fig_quality=100, active_reset=False):
        ...
        ro = -1 if active_reset else 0  # active reset: the measurement is the LAST readout trigger
        I_g = iq_list_g[self.QubitIndex][:, :, ro, 0][0]
        Q_g = iq_list_g[self.QubitIndex][:, :, ro, 1][0]
        I_e = iq_list_e[self.QubitIndex][:, :, ro, 0][0]
        Q_e = iq_list_e[self.QubitIndex][:, :, ro, 1][0]
