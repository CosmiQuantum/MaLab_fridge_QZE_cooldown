...
def sweep_frequency_offset(experiment, QubitIndex, offset_values, n_loops=10, number_of_qubits=6,
    ...
    baseline_freq = experiment.readout_cfg['res_freq_ge']
    ...
    for offset in offset_values:
        ...
        for i in range(n_loops):
            ...
            exp_copy.readout_cfg['res_freq_ge'] = offset_res_freqs[0]
...
default_qubit_freqs = [4189.8656, 3820.4723, 4161.3726, 4463.15226, 4471.446,
                       4997.86]  # from 3/11, qfreqs to fall back on if qspec is set to False
default_res_freqs = [6216.9331, 6275.9373, 6335, 6407.0338, 6476.1256,
                     6538]  # 04/07, res freqs to fall back on if res spec is set to False
default_ge_rabis = [0.6748, 0.634499, 0.76542, 0.7754, 0.6446,
                    0.9]  # from 3/11, pi amps to fall back on if g-e rabi is set to False
...
ss_keys = ['Fidelity', 'Angle', 'Dates', 'I_g', 'Q_g', 'I_e', 'Q_e', 'Round Num', 'Batch Num', 'Exp Config',
           'Syst Config']
offset_keys = ['Res Frequency', 'Fidelity', 'Angle', 'Dates', 'I_g', 'Q_g', 'I_e', 'Q_e', 'Round Num', 'Batch Num',
               'Exp Config',
               'Syst Config']
...
for QubitIndex in Qs_to_look_at:
    ...
    if run_flags["q_spec_ge"]:
        ...
        q_spec = QubitSpectroscopy(QubitIndex, number_of_qubits, optimizationFolder, j, signal, save_figs, experiment,
    ...
    if run_flags["ge_rabi"]:
        ...
        experiment.qubit_cfg['pi_amp'] = float(ge_amp_rabi)
    ...
    if run_flags["repeated_ssf"]:
        ...
        for ss_round in range(ss_sample_number):
            try:
                ss = SingleShot(QubitIndex, tot_num_of_qubits, studyDocumentationFolder, 0, False, experiment=experiment,
                                verbose=verbose, logger=rr_logger)
        ...
        experiment.readout_cfg['res_phase'] = avg_angle * 180 / np.pi  # need it to be a list of 6, the other qubits dont matter so just amke them the same val
        experiment.readout_cfg['threshold'] = avg_thresh
    ...
    if run_flags["rabi_ge_chevron"]:
        ...
        if save_data_h5:
            if save_shots_chev:
                ...
                chev_data[QubitIndex]['I'][
                    0] = all_rabi_I  # [0] is the round number, always zero since we don't use that parameter in this script
                ...
                chev_data[QubitIndex]['I'][
                    0] = all_rabi_I  # shape (n_rounds, n_freqs, n_gains). [0] is the round number, always zero since we don't use that parameter in this script.
        ...
        fig.savefig(os.path.join(studyDocumentationFolder, filename), dpi=fig_quality)
        plt.close(fig)
