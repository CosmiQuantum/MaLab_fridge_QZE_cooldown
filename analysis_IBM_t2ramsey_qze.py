...
for qubit in qubits:
    ...
    # QSpec Analysis (High Gain)
    # Note: Using high gain for n_bar calculation as it likely provides better signal
    q_vs_time_high_gain = QubitFreqsVsTime(figure_quality, final_figure_quality, 6, top_folder_dates, save_figs,
                                         False, 'None', run_name, fridge=FRIDGE, exp_name='ge', qubit=qubit)
    _, _, amps_qspec_hg, gains_qspec_hg, rounds_qspec_hg, freqs_qspec_hg = q_vs_time_high_gain.run_q_sweep_new(
        exp_extension='_ge', scaling=True, gain='_high_gain')

    # Calculate nbar
    n_bars = q_vs_time_high_gain.calculate_nbar(amps_qspec_hg, gains_qspec_hg, rounds_qspec_hg, freqs_qspec_hg, chi_MHz=-0.137)

    q_vs_time_high_gain.plot_all_q_heatmaps_new_format(amps_qspec_hg, gains_qspec_hg, rounds_qspec_hg, freqs_qspec_hg,
