...
class T2RProgram(AveragerProgramV2):
    def _initialize(self, cfg):
        ...
        self.add_pulse(ch=res_ch, name="qze_pulse",  ro_ch=ro_ch,
                       style="const",
                       length=QickSweep1D("waitloop", cfg['start']+cfg['readout_pulse_delay']+cfg['sigma'] * 4, cfg['stop']+cfg['readout_pulse_delay']+cfg['sigma'] * 4), #varying in the loop
...
class T2RMeasurementZeno:
    ...
    def t2_fit(self, x_data, I, Q, verbose = False, guess=None, plot=False, return_freq=False, mag=None):
    ...
    def run_single_gain(self, thresholding=False, correction=False, scaling=False, qze_pulse='const'):
        now = datetime.datetime.now()
        ramsey = T2RProgramSingleGain(self.experiment.soccfg, reps=self.config['reps'], final_delay=self.config['relax_delay'],
                             cfg=self.config)
        if scaling:
            q_config = all_qubit_state(self.experiment, self.number_of_qubits)
            ss_exp_cfg = add_qubit_experiment(expt_cfg, 'Readout_Optimization', self.QubitIndex)
        ...
        if self.live_plot:
            ...
            if thresholding:
                ramsey = T2RProgramSingleGain(self.experiment.soccfg, reps=self.config['reps'],
                                    final_delay=self.config['relax_delay'],
                                    cfg=self.config)
                iq_list = ramsey.acquire(self.experiment.soc, rounds=self.config['rounds'],
                                         threshold=self.experiment.readout_cfg["threshold"],
            ...
            delay_times = ramsey.get_pulse_param(pulsename='qze_pulse', parname='length', as_array=True)
            self.plot_results_interweaved_cal(Is_all, Qs_all, delay_times,  scaling=scaling, Ie = ss_I_e_all
                                  , Ig = ss_I_g_all, Qe = ss_Q_e_all, Qg = ss_Q_g_all)
            return  None, None, Is_all, Qs_all, delay_times, None, self.config, ss_Q_e_all\
    ...
    def plot_results(self, I, Q, delay_times, now, fit, t2r_est, t2r_err, plot_sig, config = None, fig_quality = 100, mag=None):
        if mag is not None:
            ...
            if self.fit_data:
                ...
                if config is not None:
                    fig.text(plot_middle, 0.98,
                             f"T2 Q{self.QubitIndex + 1}" + f", {float(config['reps'])}*{float(config['rounds'])} avgs, Zeno pulse gain {round(self.config['res_gain_qze'], 3)}",
                             fontsize=14, ha='center',
                             va='top')  # , pi gain %.2f" % float(config['pi_amp']) + f", {float(config['sigma']) * 1000} ns sigma
                else:
                    fig.text(plot_middle, 0.98,
                             f"T2 Q{self.QubitIndex + 1}, T2R %.2f us" % float(
                                 t2r_est) + f", {float(self.config['reps'])}*{float(self.config['rounds'])} avgs, Zeno pulse gain {round(self.config['res_gain_qze'], 3)}",
            ...
            # I subplot
            ax1.plot(delay_times, mag, label="Gain (a.u.)", linewidth=2)
            ...
            if self.fit_data:
                ...
                if config is not None:
                    fig.text(plot_middle, 0.98,
                             f"T2 Q{self.QubitIndex + 1}" + f", {float(config['reps'])}*{float(config['rounds'])} avgs, Zeno pulse gain {round(self.config['res_gain_qze'],3)}",
                             fontsize=14, ha='center', va='top') #, pi gain %.2f" % float(config['pi_amp']) + f", {float(config['sigma']) * 1000} ns sigma
                else:
                    fig.text(plot_middle, 0.98,
                             f"T2 Q{self.QubitIndex + 1}, T2R %.2f us" % float(
                                 t2r_est) + f", {float(self.config['reps'])}*{float(self.config['rounds'])} avgs, Zeno pulse gain {round(self.config['res_gain_qze'],3)}",
