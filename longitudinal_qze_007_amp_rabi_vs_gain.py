...
from section_006_amp_rabi_ge_zeno import AmplitudeRabiExperimentZeno
################################################ Run Configurations ####################################################
...
for QubitIndex in Qs_to_look_at:
       ...
       # Get the config for this qubit
        ...
        try:
            ...
            continue
        ################### calibrate ################
        rabi_gain_keys = ['Dates', 'I', 'Q', 'Gains', 'Fit', 'Round Num', 'Batch Num', 'Exp Config', 'Syst Config']
        ...
        ampRabiGain_data = create_data_dict(rabi_gain_keys, save_r, list_of_all_qubits)
        
        rabiStarked = AmplitudeRabiExperimentZeno(QubitIndex, tot_num_of_qubits, qubitFolder, j, signal,save_shots=False, save_figs=save_figs,
                                                   experiment=experiment, verbose=verbose,
                                                  logger=logging, expt_name='power_rabi_vs_gain')
        I_rabiStarked, Q_rabiStarked, gain_rabiStarked, q1_fit_cosine_rabiStarked, pi_amp_rabiStarked, config_rabiStarked = rabiStarked.run(
            thresholding=False, vs_res_gain_2d=True)

        ampRabiGain_data[QubitIndex]['Dates'][j - batch_num * save_r - 1] = (
            time.mktime(datetime.datetime.now().timetuple()))
        ampRabiGain_data[QubitIndex]['I'][j - batch_num * save_r - 1] = I_rabiStarked
        ampRabiGain_data[QubitIndex]['Q'][j - batch_num * save_r - 1] = Q_rabiStarked
        ampRabiGain_data[QubitIndex]['Gains'][j - batch_num * save_r - 1] = gain_rabiStarked
        ampRabiGain_data[QubitIndex]['Fit'][j - batch_num * save_r - 1] = q1_fit_cosine_rabiStarked
        ampRabiGain_data[QubitIndex]['Round Num'][j - batch_num * save_r - 1] = j
        ampRabiGain_data[QubitIndex]['Batch Num'][j - batch_num * save_r - 1] = batch_num
        ampRabiGain_data[QubitIndex]['Exp Config'][j - batch_num * save_r - 1] = expt_cfg
        ampRabiGain_data[QubitIndex]['Syst Config'][j - batch_num * save_r - 1] = config_rabiStarked

        saver_spec = Data_H5(subStudyDataFolder, ampRabiGain_data, batch_num, save_r)
        saver_spec.save_to_h5('AmpRabiResGain')
        ampRabiGain_data = create_data_dict(rabi_gain_keys, n, list_of_all_qubits)
        del saver_spec
        del rabiStarked
